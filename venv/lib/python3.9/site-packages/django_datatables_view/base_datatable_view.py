...
class DatatableMixin(object):
    ...
    pre_camel_case_notation = (
        False  # datatables 1.10 changed query string parameter names
    )
    none_string = ""
    escape_values = (
        True  # if set to true then values returned by render_column will be escaped
    )
    columns_data = []
    """
    "is_data_list" determines the type of results. If JavaScript passes data attribute that is not an integer then
    it expects dictionary with specific fields in the response, see:
    ...
    def extract_datatables_column_data(self):
        ...
        if not self.pre_camel_case_notation:
            ...
            while data_name_key in request_dict:
                searchable = (
                    True
                    if request_dict.get("columns[{0}][searchable]".format(counter))
                    == "true"
                    else False
                )
                orderable = (
                    True
                    if request_dict.get("columns[{0}][orderable]".format(counter))
                    == "true"
                    else False
                )
        ...
        if not self.pre_camel_case_notation:
            ...
            for col_no, col in enumerate(self.columns_data):
                ...
                if isinstance(data_field, int):
                    column = columns[
                        data_field
                    ]  # by index so we need columns definition in self._columns
                else:
                    column = data_field
    ...
    def count_records(self, qs):
        ...
        This method can be overwritten to optimise page loading speed.
        """
        return qs.count()
