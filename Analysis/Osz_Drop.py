...
def Osz_Drop_Analysis(Main_id):
    ...
    def get_max_of_drops(Name_col):
        ...
        for Drop_Nr in Drop_parameters['Drop Numbers']:
            smooth_data = smoothen_curve(data.loc[data['Drop_Nr']==Drop_Nr, Name_col])
    ...
    def get_min_Adv_of_drops(Name_col):
        Min_size = []
        for Drop_Nr in Drop_parameters['Drop Numbers']:
            smooth_data = smoothen_curve(data.loc[(data['Drop_Nr']==Drop_Nr)&(data['Flowrate']>0), Name_col])
            smooth_data = pd.DataFrame(data=smooth_data, columns=['smooth_data'])
            Min_size.append(get_smallest_1_per(smooth_data, 'smooth_data'))
        return Min_size
    ...
    for i in range(len(droptimes)-1):
        ...
        Drop_Nrs.append(i)
    Drop_parameters = pd.DataFrame(data=Drop_Nrs, columns=['Drop Numbers'])
    i=0
    ...
    Drop_parameters['Drop center'] = (Right_smallest_1_per_av - Left_largest_1_per_av)/2 + Left_largest_1_per_av
